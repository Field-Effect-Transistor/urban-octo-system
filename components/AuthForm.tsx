...
export default function AuthForm() {
  ...
  // Обробка входу/реєстрації через Email
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Якщо все ок - пускаємо на Дашборд
      ...
      if (err instanceof Error) {
        ...
        setError('Сталася невідома помилка. Перевірте дані.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="w-full max-w-md p-8 bg-card border border-border rounded-2xl shadow-lg">
      ...
      <form onSubmit={handleEmailAuth} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium mb-1 text-foreground/80">Пароль</label>
          <input
            type="password"
            value={password}
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full py-3 px-4 bg-primary text-white font-medium rounded-lg hover:opacity-90 transition-opacity disabled:opacity-50"
        >
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <span className="text-xs text-center text-foreground/50 uppercase">АБО ПРОДОВЖИТИ ЧЕРЕЗ</span>
        <span className="w-1/5 border-b border-border lg:w-1/4"></span>
      </div>
      ...
      <button
        ...
        type="button"
        className="mt-4 w-full flex items-center justify-center gap-2 py-3 px-4 bg-background border border-border text-foreground font-medium rounded-lg hover:bg-foreground/5 transition-colors"
      >
